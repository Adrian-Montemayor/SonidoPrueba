...
static const char		SoundCardPortName[] = "plughw:0";
...
/*Instrucciones
1-Apagado
2-Enciende   -Hace sonido de encendido
3-MB manda a MSP (1) O (2)  1- Voltaje de la batería, 2-Voltaje de la MB 
4-Si leeo un 5 pregunta ¿Quiéres apagarme?  3 para apagar 4 nada
*/
...
{	
	...
	int fd = open( portname, O_RDWR| O_NOCTTY ); //Open MSP
	if ( fd < 0 )
	{
		printf("Error de conexion\n");  /*Mensajes*/
	}
	argv[1] = "ON.wav";  // On sound
	j=1;
	printf("Welcome!\n");

	int i=1;	 /*UART Variables*/
	int n = 0,
    	spot = 0;
	char buf = '\0';	
	/* Whole response*/
	...
	do{	
		...
		if(j==2)
		{
			...
			printf("Qué voltaje desea ver?\n Presione 1 para MSP\n o \n2 Para Motherboard o 3 Para seguir\n");
			scanf("%d", &respuesta);
		if(respuesta == 1)
		{	
			j=3;		 
			write(fd, "1", 1); //Mandar 1 para voltaje de msp
			usleep ((1 + 25) * 100); 	//Dormir lo suficiente para mandar 1 Byte		      
			char buf [1024];
			n = read( fd, &buf, 1);
			if (n < 0) {
    			printf("Error al leer Voltaje MSP: \n");
			}
			else if (n == 0) {
    			printf("No leo nada del MSP: \n");
			}
			else {
			printf("Voltaje MSP: %s\n", buf);
			spot += n;
		...
		{	
			j=3;
			write(fd, "2", 1); // Mandar 2 para voltaje de mother
			usleep ((1 + 25) * 100);
			char buf [1024];
			n = read( fd, &buf, 1);
			if (n < 0) {
		...
		if( *buf == '5')
		{
			printf("Quieres apagar el msp?\n1-Presiona 3 para apagar\n2-Presione 4 para continuar\n ");
			scanf("%d", &respuesta2);
			if(respuesta2 == 3)
			{
				argv[1] = "Force_shut_down.wav";
				j=4;
				write(fd, "3", 1);
		...
		if( *buf == '6')
		{
			printf("Me estoy cargando");
		}
	printf("Sigamos sin retorno\n");		
	...
	{
		//if turnoff or force_shut_down return(0);
		// Waiting for task.
	}
	}while(respuesta2 !=3);
	return(0);
